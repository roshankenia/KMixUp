...
def k_mixup(x, y, k, alpha=1.0, use_cuda=True):
    ...
    x_new = torch.cat(x_list).cuda()
    y_a_new = torch.cat(y_a_list).cuda()
    y_b_new = torch.cat(y_b_list).cuda()

    return x_new, y_a_new, y_b_new, lam
...
# training
file = open('./checkpoint/%s_%s_%d' % (args.dataset, args.noise_type, args.k)+'_k_mixup.txt', "w")
max_test = 0
